...
- **Compare** — line-level diff between two documents, with:
  ...
  - **Download report** — saves the original (A) and changed (B) payloads exactly as pasted **in full, with no scroll clipping** (even for very large payloads), the summary stats, a symbol legend, and the full color-coded + symbol-coded diff, as **HTML**, **PDF**, or **PNG**:
  ...
  - **Download Source (A) only / Target (B) only** — saves either payload on its own as a plain file (extension auto-detected from its content), for when you want the raw document separately rather than embedded in a report
