(function(){
  ...
  function updateDetectedBadge(){
    ...
    const type = fmtType.value === 'auto' ? F.detectFormat(t) : fmtType.value;
  ...
  function runFormat(minify){
    ...
    try{
      ...
      else out = F.jsonToCsv(F.csvToJson(text));
      fmtOutput.value = out;
  ...
  document.getElementById('fmt-validate').addEventListener('click', () => {
    ...
    showStatus(fmtStatus, F.validate(text, type));
  });
  ...
  document.getElementById('cv-sample').addEventListener('click', () => { cvInput.value = SAMPLES[cvFrom.value] || SAMPLES.json; });
  ...
  const triggerCvUpload = window.ParsecDragDrop.attach(cvInput);
  ...
  document.getElementById('cv-run').addEventListener('click', () => {
    ...
    const from = cvFrom.value, to = cvTo.value, indent = cvIndent.value;
    try{
      let dataObj, out;
      ...
      if(from === 'json'){
        ...
        else out = F.jsonToCsv(dataObj);
      } else if(from === 'xml'){
        if(to === 'xml') out = F.formatXML(text, indent);
        else {
          const rootObj = F.xmlToJsonObj(text);
          if(to === 'json') out = JSON.stringify(rootObj, null, indentArg(indent));
          else if(to === 'yaml') out = window.ParsecYAML.stringify(rootObj);
          else out = F.jsonToCsv(F.extractRowsFromXmlObj(rootObj));
        }
      } else if(from === 'yaml'){
        ...
        else out = F.jsonToCsv(Array.isArray(dataObj) ? dataObj : [dataObj]);
      } else { // csv
        const rows = F.csvToJson(text);
        if(to === 'csv') out = F.jsonToCsv(rows);
