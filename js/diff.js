...
(function(global){
  ...
  // Similarity based only on alphanumeric tokens, ignoring punctuation/whitespace
  // separators like ":" or "<" that would otherwise inflate the score for any
  // two short, differently-structured lines that happen to share formatting.
  function meaningfulSimilarity(aText, bText){
    const isWord = t => /[A-Za-z0-9]/.test(t);
    const ta = tokenize(aText).filter(isWord), tb = tokenize(bText).filter(isWord);
    if(ta.length === 0 || tb.length === 0) return 0;
    const dp = tokenLCS(ta, tb);
    const lcsLen = dp[0][0];
    return (2 * lcsLen) / (ta.length + tb.length);
  ...
  function detectModifications(rows, threshold){
    ...
    for(let idx = 0; idx < rows.length - 1; idx++){
      ...
      if(cur.type === 'removed' && next.type === 'added'){
        const sim = meaningfulSimilarity(cur.text, next.text);
        if(sim >= threshold){
  ...
  global.ParsecDiff = { diffLines, detectMoves, detectModifications, tokenDiff, similarity, meaningfulSimilarity };
