...
(function(global){
  ...
  function jsonToCsv(data){
    ...
    for(const row of arr) lines.push(headers.map(h => csvEscape(row[h])).join(','));
    return lines.join('\n');
  }
  ...
  function parseAny(text, type){
    ...
    if(type === 'csv') return csvToJson(text);
  ...
  function validate(text, type){
    try{
      ...
      if(type === 'csv') return csvValidate(text);
